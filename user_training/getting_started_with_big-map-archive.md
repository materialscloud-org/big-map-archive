...
```
- Navigate to "Shared records" and click the record's title. If a data file remains unchanged from one version to the next, even though a file link appears in the two versions, the file is uploaded only once. This saves storage space and avoids unnecessary use of bandwidth. In addition to a link for each file in `data/input/upload`, you may find all of the links from the first version. This is an effect of the flag `--link-all-files-from-previous`.
